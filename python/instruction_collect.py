...
#
# python ./python_scripts/instruction_collect.py 3 ~/Software/genymotion-2.3.0/genymotion/tools/output_2013_10s/ ~/Software/genymotion-2.3.0/genymotion/tools/output_2014_10s/ ~/Software/genymotion-2.3.0/genymotion/tools/output_benign_10s/
#
            ...
            if 'instruction.out' in files:
                ...
                try:
                    for line in lines:
                        line=line.strip()
                        line=line.split('/')
                        if line[0] not in instruction_all:
                            print "add ",line[0]
                            instruction_all.append(line[0])
                        
                except:
                    print 'Error in ',apk_dir,sys.exc_info()
                    continue                    
            else:
                print 'Cannot find instruction.out ! ',folder,dirs
                continue
