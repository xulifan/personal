...
#
# python convert_apkinfo_to_feat_vect_given_name_list.py ./original_data_from_jose/malware filename_6930m 1
#
...
f_filename=open(filename_input,'r')
filename=[]
for line in f_filename.readlines():
    line=line.rstrip()
    line=line.strip()
...
for dirs in listdir(input):
    #print dirs
    apk_dir=input+'/'+dirs
    if os.path.isdir(apk_dir) and (dirs in filename):
