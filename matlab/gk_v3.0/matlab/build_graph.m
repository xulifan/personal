function graph = build_graph(filename, n_px, p)
    ...
    fprintf(1, 'LOG: image read: (%d, %d) %s\n', size(graph.img,1), size(graph.img,2),graph.filename);
    ...
    % apply the desired feature extractor
    if strcmp(p.feat_type, 'LBP')
        graph.histograms = find_lbp_features(graph);
    elseif strcmp(p.feat_type, 'SIFT')
        graph.histograms = find_sift_features(graph, p);
    ...
    if p.plot_wanted
        ...
        for i = 1 : graph.n_row_tiles
            for j = 1 : graph.n_col_tiles
                ...
                plot(graph.histograms{i,j}); set(gca,'XTickLabel',''); set(gca,'YTickLabel',''); axis tight;
                set(gca, 'LooseInset', get(gca,'TightInset'));
            end
        end
