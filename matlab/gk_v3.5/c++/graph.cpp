...
double Graph::ngk(Graph* g, ptr2nodekernel fptr, double par) {
	...
	DMatrix* vk = new DMatrix(n_nodes, g->n_nodes);
	for (int i = 0 ; i < n_nodes ; ++ i) {
		for (int j = 0 ; j < g->n_nodes ; ++ j) {
			vk->set(i, j, (*fptr)(feat_vecs->row(i),g->feat_vecs->row(j),feat_vecs->cols(),par));
		}
	}
