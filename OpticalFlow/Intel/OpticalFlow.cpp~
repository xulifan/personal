...
cudaLK::cudaLK()
{
	...
	if (0 < num_platforms) 
	{
		...
		for (unsigned i = 0; i < num_platforms; ++i) 
		{
			...
			if (strcmp(str_temp, "Advanced Micro Devices, Inc.")!=0) 
			  {
			    printf("Platform is not AMD, check next platform\n");
			  }
			else{
			  ...
			  errcode = clGetDeviceIDs( platform_id[i], CL_DEVICE_TYPE_CPU, 1, &device_id, &num_devices);
